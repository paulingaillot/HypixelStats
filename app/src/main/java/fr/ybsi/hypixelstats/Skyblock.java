...
public class Skyblock extends AppCompatActivity {
  ...
  private int[] tab = {
    ...
    97425, 147425, 222425, 322425, 522425, 822425, 1222425, 1722425, 2322425, 3022425,3822425, 4722425,
    5722425, 6822425, 8022425, 9322425, 10722425, 12222425, 13822425, 15522425, 17322425, 19222425,
    21222425, 23322425, 25522425, 27822425, 30222425, 32722425, 35322425, 38072425, 40975425,
    44072425, 47472425, 51172425, 55172425, 59472425,64072425, 68972425,74172425, 79672425, 85472425,
    91572425, 97972425, 104672425, 111672425, 111672426
  };
    ...
    protected String doInBackground(String... username) {
      try {
        ...
        for (int i = 0; i < length; i++) {
          name.add(
              i,
              SkyblockData.getAsJsonArray("profiles")
                  .get(i)
                  .getAsJsonObject()
                  .get("cute_name")
                  .toString()
  ...
  public class UpdateTask2 extends AsyncTask<String, String, String> {
    ...
    protected String doInBackground(String... username) {
      try {
          ...
          runecraft =
                  Double.parseDouble(
                          jsonObject
                                  ...
                                  .getAsJsonObject("members")
                                  .getAsJsonObject(uuid)
                                  .get("experience_skill_runecrafting")
                                  .toString());
        } catch (Exception e) {
          ...
          carpentry =
                  Double.parseDouble(
                          jsonObject
                                  ...
                                  .getAsJsonObject("members")
                                  .getAsJsonObject(uuid)
                                  .get("experience_skill_carpentry")
                                  .toString());
        } catch (Exception e) {
        ...
        farmper =
                (((int) (farm) - tab[farmlevel]) * 100) / (tab[farmlevel + 1] - tab[farmlevel]);

        mininglevel = 0;
        val = 0;
        while (mining > val && mininglevel<=60) {
          mininglevel++;
          val = tab[mininglevel];
        }
        ...
        minper =
                (((int) (mining) - tab[mininglevel]) * 100)
                        / (tab[mininglevel + 1] - tab[mininglevel]);

        combatlevel = 0;
        val = 0;
        while (combat > val && combatlevel<=60) {
          combatlevel++;
          val = tab[combatlevel];
        }
        ...
        combper =
                (((int) (combat) - tab[combatlevel]) * 100)
                        / (tab[combatlevel + 1] - tab[combatlevel]);

        foraginglevel = 0;
        val = 0;
        while (foraging > val && foraginglevel<=50) {
          foraginglevel++;
        ...
        forper =
                (((int) (foraging) - tab[foraginglevel]) * 100)
                        / (tab[foraginglevel + 1] - tab[foraginglevel]);

        runecraftinglevel = 0;
        val = 0;
        while (runecraft > val && runecraftinglevel<=25) {
          runecraftinglevel++;
        ...
        runeper =
                (((int) (runecraft) - tab[runecraftinglevel]) * 100)
                        / (tab[runecraftinglevel + 1] - tab[runecraftinglevel]);

        fishinglevel = 0;
        val = 0;
        while (fish > val && fishinglevel<=50) {
          fishinglevel++;
        ...
        fishper =
                (((int) (fish) - tab[fishinglevel]) * 100)
                        / (tab[fishinglevel + 1] - tab[fishinglevel]);

        enchantinglevel = 0;
        val = 0;
        while (enchant > val && enchantinglevel<=60) {
          enchantinglevel++;
        ...
        enchantper =
                (((int) (enchant) - tab[enchantinglevel]) * 100)
                        / (tab[enchantinglevel + 1] - tab[enchantinglevel]);

        alchemylevel = 0;
        val = 0;
        while (alchemy > val && alchemylevel<=50) {
          alchemylevel++;
        ...
        alcper =
                (((int) (alchemy) - tab[alchemylevel]) * 100)
                        / (tab[alchemylevel + 1] - tab[alchemylevel]);

        taminglevel = 0;
        val = 0;
        while (taming > val && taminglevel<=50) {
          taminglevel++;
          val = tab[taminglevel];
        }
        ...
        tamper =
                (((int) (taming) - tab[taminglevel]) * 100)
                        / (tab[taminglevel + 1] - tab[taminglevel]);

        carpentrylevel = 0;
        val = 0;
        while (carpentry > val && carpentrylevel<=50) {
          carpentrylevel++;
        ...
        carper =
                (((int) (carpentry) - tab[carpentrylevel]) * 100)
                        / (tab[carpentrylevel + 1] - tab[carpentrylevel]);
